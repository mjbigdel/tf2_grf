...
from dqn_ma_4_FP.dqn_agent import MAgent
    ...
    **network_kwargs
    ...
    for t in range(total_timesteps):
        ...
        for _ in range(nsteps):
            ...
            actions_list, fps_ = dqn_agent.choose_action(tf.constant(obs_all), update_eps=update_eps, **kwargs)
            # print(np.array(fps_).shape)
            fps = []
            for a in agent_ids:
        ...
        # nseconds = time.time() - steps_start
        # print(f'time spend to perform {nsteps} steps is {nseconds} ')
        ...
        if replay_buffer is not None:
            replay_buffer.add(mb_obs, mb_actions, mb_rewards, obs1_all, mb_masks,
                              mb_fps, np.tile([exploration.value(t), t], (nsteps, num_agents, 1)))
        ...
        if t > learning_starts and t % train_freq == 0:
            ...
            s = obses_tp1.shape
            # print(f'obses_t.shape is {s}')
            obses_tp1 = tf.reshape(obses_tp1, (s[0], *s[1:]))  # obses_tp1 is single observation after last step
            s = actions.shape
        ...
        # print(f'time spend to perform 1 run is {nseconds} ')
