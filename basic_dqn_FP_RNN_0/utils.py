...
def init_network(config):
    ...
    inputs_obs = tf.keras.Input(shape=config.obs_shape, name='0')  # , name='1'
    ...
    done_Input = tf.keras.Input(shape=(config.n_steps), name='3')
    ...
    for i, (num_ch, num_blocks) in enumerate(conv_layers):
        ...
        for j in range(num_blocks):
            with tf.name_scope('residual_%d_%d' % (i, j)):
                ...
                conv_out = tf.keras.layers.BatchNormalization()(conv_out)
                conv_out = tf.keras.layers.Activation(tf.nn.relu, name=f'act_{i}_{j}_2')(conv_out)
                conv_out = tf.keras.layers.Conv2D(filters=num_ch, kernel_size=3, strides=(1, 1), padding='same',
                                                  name=f'conv2_{i}_{j}_2', kernel_initializer=kernel_init,
                                                  bias_initializer=bias_init)(conv_out)
                conv_out = tf.keras.layers.BatchNormalization()(conv_out)
                conv_out = tf.keras.layers.Activation(tf.nn.relu, name=f'act_{i}_{j}_3')(conv_out)
                conv_out += block_input

    conv_out = tf.keras.layers.Activation(tf.nn.relu)(conv_out)
    conv_out = tf.keras.layers.BatchNormalization()(conv_out)
    conv_out = tf.keras.layers.Flatten()(conv_out)
    conv_out = tf.keras.layers.concatenate([conv_out, inputs_agent_name_oh])
    ...
    reshaped = tf.reshape(outputs, shape=(-1, config.n_steps, outputs.shape[-1]))
    outputs = tf.keras.layers.LSTM(units=512, dropout=0.25, return_sequences=False)(inputs=reshaped, mask=masking)
