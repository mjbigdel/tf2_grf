...
def build_ma_cnn(input_shape, num_actions, agents_names, fc1_dims, fc2_dims, share_weights, model_name):
    ...
    if share_weights:
        ...
        for agent_name in agents_names:
            dense1[agent_name] = tf.keras.layers.Dense(fc1_dims, activation=tf.nn.relu, name=f'dense1_{agent_name}')(flatten)
            outputs[agent_name] = tf.keras.layers.Dense(num_actions, name=f'output_{agent_name}')(dense1[agent_name])
