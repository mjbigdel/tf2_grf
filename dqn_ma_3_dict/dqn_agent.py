...
from dqn_ma_3_dict.tf2_models import build_ma_mlp, build_ma_cnn, build_cnn, build_mlp
...
class MAgent:
    def __init__(self, env, agent_names, lr=0.0005, replay_buffer=None, shared_weights=False, double_q=False,
        ...
        tf.keras.utils.plot_model(self.q_network, to_file='./model.png')
        with tf.name_scope('shared_target_q_network'):
            self.target_q_network = build_ma_cnn(self.obs_shape, self.num_actions,
                                                 self.agent_names, 512, 512, self.shared_weights, 'shared_target_q_network')
    ...
    def train(self, obs0, actions, rewards, obs1, dones, importance_weights, agent_name):
        ...
        # print(f'params for {agent_name} is {param}')
