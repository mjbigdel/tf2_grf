...
from dqn_ma_3_dict.dqn_agent import MAgent
...
def play_test_games(dqn_agent):
    ...
    render = True
    stacked = True
    env_name = 'academy_3_vs_1_with_keeper'  #'academy_3_vs_1_with_keeper'
    channel_dim = (42, 42)
    ...
    num_tests = 3
    agent_0 = test_env.agent_names()[0]
    ...
    for i in range(num_tests):
        ...
        while not test_done:
            ...
            if test_done:
                print(f'test_reward_dict for test {i} is {test_rew_dict}')
    ...
    **network_kwargs
    ...
    if replay_memory:
        ...
        for agent_name in env.agent_names():
            if prioritized_replay:
                replay_buffer[agent_name] = PrioritizedReplayBuffer(buffer_size, alpha=prioritized_replay_alpha)
                ...
                replay_buffer[agent_name] = ReplayBuffer(buffer_size)
                beta_schedule = None
    else:
        replay_buffer = None
        beta_schedule = None
    ...
    for t in range(total_timesteps):
        ...
        if t % 1000 == 0:
            ...
            print(f'time spend to perform {t-1000} to {t} steps is {nseconds} ')
        ...
        reset = False
        new_obs_dict, rew_dict, done_dict, _ = env.step(action_dict)
        done = done_dict['__all__']
        if replay_buffer is not None:
            # Store transition in the replay buffer.
            for agent_name in agent_names:
                replay_buffer[agent_name].add(obs_dict[agent_name], action_dict[agent_name],
                                              rew_dict[agent_name], new_obs_dict[agent_name], float(done))

        obs = new_obs_dict

        episode_rewards[-1] += rew_dict[agent_names[0]]
        if done:
            obs_dict = env.reset()
        ...
        if t > learning_starts and t % train_freq == 0:
            ...
            for i, agent_name in enumerate(agent_names):
                if prioritized_replay:
                    experience = replay_buffer[agent_name].sample(batch_size, beta=beta_schedule.value(t))
                    (obses_t, actions, rewards, obses_tp1, dones, weights, batch_idxes) = experience
                else:
                    obses_t, actions, rewards, obses_tp1, dones = replay_buffer[agent_name].sample(batch_size)
            ...
            if prioritized_replay:
                ...
                replay_buffer[agent_name].update_priorities(batch_idxes, new_priorities)
