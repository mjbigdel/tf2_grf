...
while not target_reached and iters < max_iters:
    ...
    for itr in range(ppo_steps):
        ...
        state = observation
        if done:
            # print('action=' + str(action) + ', reward=' + str(reward) + ', q val=' + str(q_value))
            print(
    ...
    actor_loss = model_actor.fit(
        x=[np.asarray(states), np.asarray(actions_probs), np.asarray(advantages),
           np.reshape(rewards, newshape=(-1, 1, 1)), np.asarray(values[:-1])],
        y=np.reshape(actions_onehot, newshape=(-1, n_actions)),
        verbose=False,
        shuffle=True,
        epochs=8,
    ...
    critic_loss = model_critic.fit([states], [np.reshape(returns, newshape=(-1, 1))],
                                   shuffle=True, epochs=8, verbose=False,
                                   # callbacks=[tensor_board]
                                   )
