...
from basic_dqn_FP_RNN_1.utils import init_replay_memory, init_network_2, init_network
...
class Agent(tf.Module):
    ...
    def initialize_dummy_variabels(self):
        ...
        self.dummy_fps = tf.zeros((1, 1, (self.config.num_agents - 1) * self.config.num_actions + self.config.num_extra_data))
    ...
    def value(self, obs):
        ...
        for a in self.agent_ids:
            ...
            q_values = self.target_agent_heads[a](fc_values[:, -1, :]) # [:, -1, :]

            if self.config.double_q:
                fc_values_using_online_net = self.model(inputs)
                q_values_using_online_net = self.agent_heads[a](fc_values_using_online_net[:, -1, :]) # [:, -1, :]
    ...
    def learn(self):
        ...
        for t in range(self.config.num_timesteps):
            ...
            if t % (self.config.print_freq) == 0:
                ...
                print(f'eps_update {self.exploration.value(t)} -- time {t - self.config.print_freq} to {t} steps: {nseconds} ')
