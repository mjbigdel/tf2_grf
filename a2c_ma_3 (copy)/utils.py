...
def conv(scope, *, nf, rf, stride, activation, pad='valid', init_scale=1.0, data_format='channels_last'):
    ...
    #     layer =
    return tf.keras.layers.Conv2D(filters=nf, kernel_size=rf, strides=stride, padding=pad, data_format=data_format,
                                  kernel_initializer=ortho_init(init_scale), name=scope, activation=activation)


def fc_build(input_shape, scope, nh, *, init_scale=1.0, init_bias=0.0):
    # with tf.name_scope(scope):
    layer = tf.keras.layers.Dense(units=nh, kernel_initializer=ortho_init(init_scale),
                                  bias_initializer=tf.keras.initializers.Constant(init_bias), name=scope)
    layer.build(input_shape)
    return layer


def fc(scope, nh, *, init_scale=1.0, init_bias=0.0):
    # with tf.name_scope(scope):
    #     layer =
    return tf.keras.layers.Dense(units=nh, kernel_initializer=ortho_init(init_scale),
                                 bias_initializer=tf.keras.initializers.Constant(init_bias), name=scope)
