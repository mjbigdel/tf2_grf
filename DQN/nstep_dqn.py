...
from DQN.tf2_models import build_q_func, gfootball_impala_cnn
from DQN.utils import play_test_games
from DQN.agents import Agent, MAgent
...
def learn(env,
    ...
    **network_kwargs
    ...
    network = gfootball_impala_cnn(obs_shape, len(agent_ids), 'nstep_dqn_cnn')
    q_func = build_q_func(network, hiddens=hiddens, dueling=dueling, layer_norm=layer_norm)

    if len(agent_ids) == 1:
        dqn_agent = Agent(q_func, obs_shape, agent_ids, lr, tau, double_q, num_actions, gamma,
    ...
    for t in range(total_timesteps):
        ...
        for _ in range(nsteps):
            ...
            obs_ = tf.constant(obs)
            actions_list = dqn_agent.choose_action(obs_, update_eps=update_eps, **kwargs)
        ...
        if replay_buffer is not None:
            replay_buffer.add(mb_obs, mb_actions, mb_rewards, obs1, mb_masks,
                              fc_values, np.tile([exploration.value(t), t], (len(agent_ids), 1)))
        ...
        if t > learning_starts and t % train_freq == 0:
            ...
            loss, td_errors = dqn_agent.nstep_train(obses_t, actions, rewards, obses_tp1, dones, weights, fps, extra_datas)
