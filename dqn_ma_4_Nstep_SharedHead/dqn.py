    ...
    **network_kwargs
    ...
    for t in range(total_timesteps):
        ...
        if t > learning_starts and t % train_freq == 0:
            ...
            weights, fps, extra_datas = tf.constant(weights), None, None

            s = obses_t.shape
            # print(f'obses_t shape is {s}')
            obses_t = tf.reshape(obses_t, (s[0] * s[1] * s[2], *s[3:]))
            s = actions.shape
            actions = tf.reshape(actions, (s[0] * s[1] * s[2], *s[3:]))
            s = rewards.shape
            rewards = tf.reshape(rewards, (s[0] * s[1] * s[2], *s[3:]))
            s = weights.shape
            weights = tf.reshape(weights, (s[0] * s[1] * s[2], *s[3:]))
