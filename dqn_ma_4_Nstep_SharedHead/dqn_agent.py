...
class MAgent(tf.Module):
    def __init__(self, env, agent_ids, lr=0.0005, replay_buffer=None, shared_weights=False, double_q=False,
        ...
        self.q_fc = self._build_q_head()

        self.target_q_fc = self._build_q_head()
        self.target_q_fc.trainable = False
    ...
    def _build_q_head(self):
        ...
        name = 'q'
        return fc_build(input_shape, name, self.num_actions)
    ...
    def choose_action(self, obs, stochastic=True, update_eps=-1):
        if self.param_noise:
            ...
            fc_values = self.value_network({0: obs, 1: self.one_hot_agents})
            q_values = self.q_fc(fc_values)
            deterministic_actions = tf.argmax(q_values, axis=1)
            ...
            if stochastic:
                output_actions = stochastic_actions.numpy()
            else:
                output_actions = deterministic_actions.numpy()
    ...
    def value(self, obs):
        ...
        if self.param_noise:
            ...
            fc_values = self.target_network({0: obs, 1: self.one_hot_agents})
            q_tp1 = self.target_q_fc(fc_values)
            q_tp1_best = tf.reduce_max(q_tp1, 1)
        return q_tp1_best.numpy()
    ...
    def nstep_train(self, obs0, actions, rewards, obs1, dones, importance_weights, fps, extra_datas):
        ...
        with tf.GradientTape() as tape:
            fc_values = self.value_network({0: obs0, 1: tf.tile(self.one_hot_agents, (tile_time ,1, 1))})
            q_t = self.q_fc(fc_values)
    ...
    def choose_greedy_action(self, obs_all):
        if self.param_noise:
            ...
            fc_values = self.value_network({0: obs_all, 1: self.one_hot_agents})
            q_values = self.q_fc(fc_values)
            deterministic_actions = tf.argmax(q_values, axis=1)

            return deterministic_actions.numpy()
    ...
    def update_target(self):
        ...
        q_fc_vars = self.q_fc.trainable_variables
        target_q_fc_vars = self.target_q_fc.trainable_variables
