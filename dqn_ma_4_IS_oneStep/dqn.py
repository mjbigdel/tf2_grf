...
from dqn_ma_4_IS_oneStep.dqn_agent import MAgent
...
def play_test_games(dqn_agent):
    ...
    render = True
    stacked = True
    env_name = 'academy_empty_goal_close'  #'academy_3_vs_1_with_keeper'
    channel_dim = (51, 40)
...
def learn(env,
    ...
    **network_kwargs
    ...
    dqn_agent = MAgent(env, agent_ids, lr, replay_buffer, shared_weights, double_q, num_actions,
    ...
    for t in range(total_timesteps):
        ...
        if t % 100 == 0:
            ...
            print(f'time spend to perform {t-100} to {t} steps is {nseconds} ')
            print('eps update', exploration.value(t))

        # obs is a list of all agents observations
        obs_ = tf.constant(obs_all)
        actions_list, fc_values = dqn_agent.choose_action(obs_, update_eps=update_eps, **kwargs)
        # print(f'action is {actions_list}')

        reset = False
        new_obs_all, rew, done, _ = env.step(actions_list)

        if replay_buffer is not None:
            replay_buffer.add(obs_all, actions_list, rew, new_obs_all, float(done),
                              fc_values, np.tile([exploration.value(t), t], (num_agents, 1)))
        ...
        if t > learning_starts and t % train_freq == 0:
            ...
            # for a in agent_ids:
            td_errors += dqn_agent.train(obses_t, actions, rewards, obses_tp1, dones, weights, fps, extra_datas)
