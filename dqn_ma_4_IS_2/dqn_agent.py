...
class MAgent(tf.Module):
    ...
    def _build_q_head(self):
        ...
        if self.agent_ids is not None:
            name += '_agent_' + str(self.agent_ids)
    ...
    def choose_action(self, obs, stochastic=True, update_eps=-1):
        if self.param_noise:
            ...
            fc_values = self.value_network(obs)
            q_values = self.q_fc_list[0](fc_values)
            for a in self.agent_ids:
                if a == 0:
                    continue
                q_values += self.q_fc_list[a](fc_values)
    ...
    def choose_greedy_action(self, obs_all):
        if self.param_noise:
            ...
            fc_values = self.value_network(obs_all)
            q_values = self.q_fc_list[0](fc_values)
            for a in self.agent_ids:
                if a == 0:
                    continue
    ...
    def update_target(self):
        ...
        target_q_fc_vars = []
        for a in self.agent_ids:
            q_fc_vars += self.q_fc_list[a].trainable_variables
            target_q_fc_vars += self.target_q_fc_list[a].trainable_variables
