...
class MAgent(tf.Module):
    def __init__(self, q_func, obs_shape, agent_ids, lr=0.0005, tau=0.01, double_q=False,
        ...
        self.one_hot_agents = tf.expand_dims(tf.one_hot(self.agent_ids, len(self.agent_ids),
                                                        dtype=tf.float32), axis=1)
        print(f'self.one_hot_agents is {self.one_hot_agents}')
        self.dummy_fps = np.zeros((1, len(self.agent_ids)-1, self.num_actions))
        self.dummy_extra_data = np.zeros((1, 2))
    ...
    def choose_action(self, obs, stochastic=True, update_eps=-1):
        ...
        if self.param_noise:
            ...
            for a in self.agent_ids:
                ...
                q_values = self.q_network({0: tf.expand_dims(obs[a], 0),
                                           1: self.one_hot_agents[a],
                                           2: self.dummy_fps,
                                           3: self.dummy_extra_data
                                           })[a]
    ...
    def value(self, obs):
        ...
        if self.param_noise:
            ...
            for a in self.agent_ids:
                q_tp1 = self.target_q_network({0: tf.expand_dims(obs[a], 0),
                                               1: self.one_hot_agents[a],
                                               2: self.dummy_fps,
                                               3: self.dummy_extra_data
                                               })[a]
                ...
                if self.double_q:
                    q_tp1_using_online_net = self.q_network({0: tf.expand_dims(obs[a], 0),
                                                             1: self.one_hot_agents[a],
                                                             2: self.dummy_fps,
                                                             3: self.dummy_extra_data
                                                             })[a]
    ...
    @tf.function()
    def nstep_train(self, obs0, actions, rewards, dones, importance_weights, fps, extra_datas):
        batch_size = obs0.shape[0]
        td_error_ = []
        loss = []
        with tf.GradientTape() as tape:
            for a in self.agent_ids:
                q_t = self.q_network({0: obs0[:, a, :],
